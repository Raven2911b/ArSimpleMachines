...
public class PrecisionAssemblerMenu extends AbstractContainerMenu {

    private PrecisionAssemblerControllerBlockEntity blockEntity;
    private final ContainerLevelAccess access;
    private final BlockPos pos;

    public PrecisionAssemblerMenu(int windowId, Inventory playerInv, FriendlyByteBuf buf) {
    ...
    public PrecisionAssemblerMenu(int windowId, Inventory playerInv, BlockPos pos) {
        super(ModMenuTypes.PRECISION_ASSEMBLER_MENU.get(), windowId);
        this.pos = pos;
        this.access = ContainerLevelAccess.create(playerInv.player.level(), pos);

        if (playerInv.player.level().getBlockEntity(pos) instanceof PrecisionAssemblerControllerBlockEntity e) {
            this.blockEntity = e;
        } else {
            this.blockEntity = null;
        }

        // No machine slots — assembler uses external IO blocks only
        addPlayerInventory(playerInv);
        addPlayerHotbar(playerInv);
    }

    public PrecisionAssemblerControllerBlockEntity getBlockEntity() {
        return blockEntity;
    }

    @Override
    public boolean stillValid(Player player) {
        return stillValid(access, player, ModBlocks.PRECISION_ASSEMBLER_CONTROLLER.get());
    }
